...
function extractJOInfo(text: string): JOInfo {
  ...
  const isSpecial =
    text.toLowerCase().includes("spécial") ||
    text.toLowerCase().includes("special");
  const type = isSpecial ? "Spécial" : "Ordinaire";

  console.log(
    `Informations extraites: Numéro=${numero}, Date=${date}, Type=${type}`,
  );
...
function identifyMinisteres(text: string): string[] {
  ...
  const ministeresPattern =
    /\b(MINISTÈRE|MINISTERE|PRESIDENCE|PRIMATURE|ASSEMBLÉE|ASSEMBLEE|CONSEIL|COUR|SECRÉTARIAT|SECRETARIAT)[A-ZÉÈÊËÀÂÄÔÖÙÛÜÇ\s,''()-]+\b/g;
  const matches = text.match(ministeresPattern) || [];

  // Nettoyer les résultats
  const ministeres = matches
    .map((m) => m.trim())
...
function extractActesJuridiques(
  text: string,
  ministere: string,
): ActeJuridique[] {
  ...
  const ministereSectionRegex = new RegExp(
    `${ministere.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}[\\s\\S]*?(?=(MINISTÈRE|MINISTERE|PRESIDENCE|PRIMATURE|ASSEMBLÉE|ASSEMBLEE|CONSEIL|COUR|SECRÉTARIAT|SECRETARIAT)[A-ZÉÈÊËÀÂÄÔÖÙÛÜÇ\\s,''()-]+|PARTIE NON OFFICIELLE|$)`,
    "i",
  );
  ...
  const actesPattern =
    /(Décret|Arrêté|Arrete|Loi|Ordonnance|Décision|Decision)[\s\S]*?n[°º]\s*(\d+[-\d]*)[^\n.]*du\s+(\d{1,2}[^\d\n]*\d{4})[^\n]*/gi;
  ...
  while ((match = actesPattern.exec(sectionText)) !== null) {
    ...
    const dateMatch = dateTexte.match(
      /(\d{1,2})[er]?\s+([a-zéûôêç]+)\s+(\d{4})/i,
    );
    let date = null;
...
function extractPartieNonOfficielle(text: string): PartieNonOfficielle | null {
  ...
  console.log(
    `${elements.length} éléments extraits dans la partie non officielle`,
  );

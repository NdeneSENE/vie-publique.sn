...
// Configuration pour pdf.js en environnement Node
...
async function extractTextFromPage(
  ...
  try {
    ...
    // Créer un canvas pour rendre la page (en utilisant node-canvas)
    ...
    console.error(
      `Erreur lors de l'extraction du texte de la page ${pageNum}:`,
      error,
    );
    return "";
  }
...
async function extractSommaire(pdfPath: string): Promise<string[]> {
  try {
    ...
    console.log(
      `Analyse du document ${path.basename(pdfPath)} (${numPages} pages)...`,
    );
  ...
  try {
    ...
    for (const file of pdfFiles) {
      ...
      const outputPath = path.join(
        SOMMAIRE_OUTPUT_DIR,
        `${fileNameWithoutExt}-sommaire.txt`,
      );
      ...
      if (sommaire.length > 0) {
        ...
        console.log(
          `Sommaire extrait et enregistré sous ${fileNameWithoutExt}-sommaire.txt`,
        );
      } else {

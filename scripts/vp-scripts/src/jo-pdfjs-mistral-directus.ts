...
if (
  ...
  console.error(
    "Erreur: Les variables d'environnement DIRECTUS_URL, DIRECTUS_ADMIN_TOKEN et MISTRAL_API_KEY doivent être définies dans le fichier .env",
  );
  process.exit(1);
}
...
async function extractTextFromPdfFirstPages(
  pdfPath: string,
  numPagesToExtract: number = 2,
): Promise<string[]> {
  try {
    console.log(
      `Extraction du texte des ${numPagesToExtract} premières pages du fichier: ${pdfPath}`,
    );
    ...
    console.error(
      `Erreur lors de l'extraction du texte du PDF ${pdfPath}:`,
      error,
    );
    return [];
  }
...
`;
    ...
    if (
      ...
      console.error(
        "Format de réponse Mistral inattendu:",
        JSON.stringify(data),
      );
      return null;
    }
    ...
    try {
      ...
      const sommaire: Sommaire = {
        ...
        partieOfficielle: parsedData.partieOfficielle.map(
          (section: MistralSection) => ({
            nom: section.nom,
            actes: Array.isArray(section.actes)
              ? section.actes.map((acte: MistralActe) => ({
                  type: acte.type,
                  numero: acte.numero,
                  date: acte.date,
      ...
      console.error(
        "Erreur lors de l'analyse de la réponse JSON de Mistral:",
        parseError,
      );
      return null;
    }
...
async function uploadPdfToDirectus(filePath: string): Promise<string | null> {
  try {
    ...
    if (data && data.data && data.data.id) {
      console.log(
        `Fichier ${fileName} uploadé avec succès, ID: ${data.data.id}`,
      );
      return data.data.id;
    } else {
      console.error(
        `Réponse inattendue lors de l'upload de ${fileName}:`,
        JSON.stringify(data),
      );
      return null;
    }
    ...
    console.error(
      `Erreur détaillée lors de l'upload du fichier ${filePath}:`,
      error,
    );
    return null;
  }
...
async function createDocumentInDirectus(
  ...
  try {
    console.log(
      `Création du document dans Directus pour le JO n°${sommaire.info.numero || "inconnu"}`,
    );
    ...
    if (date) {
      ...
      if (!publishDate) {
        ...
        console.error(
          `Impossible de créer le document: format de date non reconnu. Veuillez vérifier le format de la date dans le PDF ou le nom du fichier.`,
        );
        console.error(
          `Formats de date acceptés: JJ/MM/AAAA, YYYY-MM-DD, ou JJ MMM AAAA (ex: 05 JAN 2023)`,
        );
        return null;
      }
    } else {
      ...
      console.error(
        `Aucune date n'a été extraite du document. Impossible de créer l'entrée dans Directus.`,
      );
      console.error(
        `Veuillez vérifier le PDF ou ajouter manuellement la date dans le nom du fichier.`,
      );
      return null;
    }
    ...
    const detailedDescription =
      generateDetailedDescription(sommaire).slice(0, 240) + "...";
    ...
    console.log(
      `Tentative de création du document dans la collection "documents"...`,
    );
    ...
    if (documentResponse && documentResponse.id) {
      ...
      console.log(
        `Document créé avec succès: Journal Officiel n°${numero} du ${day}/${month}/${year}`,
      );
      return documentResponse.id;
    } else {
      console.error(
        "Réponse inattendue lors de la création du document:",
...
function extractDateFromFileName(fileName: string): string | null {
  console.log(
    `Tentative d'extraction de la date du nom de fichier: ${fileName}`,
  );
  ...
  if (match1) {
    ...
    console.log(
      `Date extraite du nom de fichier (format JJ/MM/AAAA): ${day}/${month}/${year}`,
    );
  ...
  if (match2) {
    ...
    console.log(
      `Date extraite du nom de fichier (format JJ MMM AAAA): ${day} ${monthText} ${year}`,
    );
  ...
  if (match3) {
    ...
    console.log(
      `Date extraite du nom de fichier (format AAAA/MM/JJ): ${day}/${month}/${year}`,
    );
    return `${day}/${month}/${year}`;
  }
...
async function processJOPdf(pdfPath: string): Promise<boolean> {
  try {
    ...
    const documentId = await createDocumentInDirectus(
      fileId,
      sommaire,
      htmlContent,
    );
    ...
    if (!documentId) {
      console.error(
        `Échec de la création du document dans Directus pour ${pdfPath}`,
      );
      return false;
    }
    ...
    try {
      ...
      console.error(
        `Erreur lors de la suppression du fichier ${pdfPath}:`,
        deleteError,
      );
...
async function processAllJOPdfs() {
  try {
    ...
    if (pdfFiles.length === 0) {
      console.log(
        "Aucun fichier PDF trouvé dans le répertoire input-sommaire.",
      );
      return;
    }
    ...
    if (successfullyProcessedFiles.length > 0) {
      console.log(
        `Suppression de ${successfullyProcessedFiles.length} fichiers traités avec succès...`,
      );
    ...
    if (errorCount === 0) {
      console.log(
        `Traitement terminé avec succès! ${successCount} fichiers traités et supprimés.`,
      );
    } else {
      console.log(
        `Traitement terminé avec des erreurs: ${successCount} fichiers traités avec succès et supprimés, ${errorCount} échecs.`,
      );
    }
  } catch (error) {

...
if (!process.env.DIRECTUS_URL || !process.env.DIRECTUS_ADMIN_TOKEN) {
  console.error(
    "Erreur: Les variables d'environnement DIRECTUS_URL et DIRECTUS_ADMIN_TOKEN doivent être définies dans le fichier .env",
  );
  process.exit(1);
}
  ...
  try {
    ...
    for (const file of pdfFiles) {
      try {
        ...
        const { number, date, slug } =
          extractInfoFromFilename(fileNameWithoutExt);

        if (!number || !date) {
          console.warn(
            `Impossible d'extraire les informations nécessaires du fichier ${file}`,
          );
          errorCount++;
          continue;
        }
        ...
        try {
          console.log(
            `Tentative de création du document avec type="official_journal"...`,
          );
          ...
          console.log(
            `Document créé avec succès: Journal Officiel n°${number} du ${date.split("-").reverse().join("/")}`,
          );
          successCount++;
          ...
          if (
            documentError &&
            typeof documentError === "object" &&
            "errors" in documentError
          ) {
            console.error(
              "Détails de l'erreur:",
              JSON.stringify(
                (documentError as { errors: unknown }).errors,
                null,
                2,
              ),
            );

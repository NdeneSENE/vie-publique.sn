...
function extractDateFromTitle(
  title: string,
): { day: string; monthText: string; year: string } | null {
...
async function updateMissingFields() {
  try {
    console.log(
      "Récupération des documents officiels publiés sans slug ou jo_number...",
    );
    ...
    for (const document of data.data) {
      try {
        ...
        if (!document.title) {
          console.warn(
            `Le document ${document.id} n'a pas de titre. Impossible de continuer.`,
          );
          errorCount++;
          continue;
        }
        ...
        if (!joNumber || !dateInfo) {
          console.warn(
            `Impossible d'extraire les informations nécessaires du titre: ${document.title}`,
          );
          errorCount++;
          continue;
        }
        ...
        console.log(
          `Mise à jour du document ${document.id} avec:`,
        ...
        console.error(
          `Erreur lors de la mise à jour du document ${document.id}:`,
          error,
        );
        errorCount++;
      }

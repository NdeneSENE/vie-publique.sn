...
async function extractFirstPagesAsImages(
  pdfPath: string,
  numPages: number = 2,
): Promise<string[]> {
  try {
    console.log(
      `Extraction des ${numPages} premières pages en images: ${pdfPath}`,
    );
    ...
    console.error(
      `Erreur lors de l'extraction des images du PDF ${pdfPath}:`,
      error,
    );
    return [];
  }
  ...
  try {
    ...
    await worker.setParameters({
      tessedit_char_whitelist:
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzÀÂÇÈÉÊËÎÏÔÛÙÜàâçèéêëîïôûùü0123456789/-.,()[]{}°'\"",
    ...
    try {
      ...
      const sommaire: Sommaire = {
        ...
        partieOfficielle: parsedData.partieOfficielle.map(
          (section: SectionMinistere) => ({
            nom: section.nom,
            actes: Array.isArray(section.actes)
              ? section.actes.map((acte: ActeJuridique) => ({
                  type: acte.type,
                  numero: acte.numero,
                  date: acte.date,
...
async function processAllJOPdfs() {
  try {
    ...
    if (errorCount === 0) {
      console.log(
        `Traitement terminé avec succès! ${successCount} fichiers traités.`,
      );
    } else {
      console.log(
        `Traitement terminé avec des erreurs: ${successCount} fichiers traités avec succès, ${errorCount} échecs.`,
      );
    }
  } catch (error) {

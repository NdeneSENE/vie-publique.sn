...
if (!process.env.MISTRAL_API_KEY) {
  console.error(
    "Erreur: La variable d'environnement MISTRAL_API_KEY doit être définie dans le fichier .env",
  );
  process.exit(1);
}
...
async function processAllJOPdfs() {
  try {
    console.log(
      "Démarrage du traitement des Journaux Officiels avec Mistral AI...",
    );
